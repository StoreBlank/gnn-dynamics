...
def visualize_graph(data_dir, episode_idx, start, end, vis_t, save_dir,
        kp_vis, gt_kp_vis, dynamic_tool_kp, static_tool_kp, Rr, Rs,
        max_nobj, max_ntool,
...
def construct_graph(dataset, n_his, pair, episode_idx, physics_param, material_config):
    ...
    for i in range(len(pair)):
        frame_idx = pair[i]
        obj_kp, static_tool_kp, dynamic_tool_kp = extract_kp_single_frame(dataset["data_dir"], episode_idx, frame_idx)
    ...
    # get current state delta
    static_tool_kp = np.stack(static_tool_kps[n_his-1:n_his+1], axis=0)  # (2, N, 3)
    dynamic_tool_kp = np.stack(dynamic_tool_kps[n_his-1:n_his+1], axis=0)  # (2, N, 3)
    static_tool_kp_num, dynamic_tool_kp_num = static_tool_kp.shape[1], dynamic_tool_kp.shape[1]

    tool_kp = np.concatenate([static_tool_kp, dynamic_tool_kp], axis=1) # (2, N, 3)
    ...
    attrs[max_nobj : max_nobj + dynamic_tool_kp_num] = 1 # dynamic tool
    attrs[max_nobj + max_ntool : max_nobj + max_ntool + static_tool_kp_num] = 1 # static tool
    ...
    graph = {
        ...
        # for non-model use
        "tool_kp": tool_kp,  # (2, static_tool_num + dynamic_tool_num, 3)
    }
...
def rollout_from_start_graph(graph, model, material_config, device, dataset, episode_idx, current_start, current_end, 
    ...
    if vis:
        Rr = graph['Rr'].numpy()
        Rs = graph['Rs'].numpy()
        dynamic_tool_kp = graph['dynamic_tool_kp'].numpy()
        static_tool_kp = graph['static_tool_kp'].numpy()
        ...
        pred_kp_proj_last, gt_kp_proj_last, gt_lineset, pred_lineset = \
            ...
            kp_vis, kp_vis, dynamic_tool_kp, static_tool_kp, Rr, Rs, max_nobj, max_ntool)
    ...
    with torch.no_grad():
        for i in range(1, 1 + rollout_steps):
            ...
            # prepare gt
            gt_state, _, _ = extract_kp_single_frame(dataset["data_dir"], episode_idx, current_end)
            gt_state = [gt_state]
            ...
            # generate next graph
            # load tool kypts
            _, static_tool_kp_start, dynamic_tool_kp_start = extract_kp_single_frame(dataset["data_dir"], episode_idx, current_start)
            _, static_tool_kp_end, dynamic_tool_kp_end = extract_kp_single_frame(dataset["data_dir"], episode_idx, current_end)

            static_tool_kp_num = static_tool_kp_start.shape[0]
            dynamic_tool_kp_num = dynamic_tool_kp_start.shape[0]

            static_tool_kp = np.stack([static_tool_kp_start, static_tool_kp_end], axis=0)  # (2, N, 3)
            dynamic_tool_kp = np.stack([dynamic_tool_kp_start, dynamic_tool_kp_end], axis=0)  # (2, N, 3)
            static_tool_kp = pad(static_tool_kp, max_ntool, dim=1)
            dynamic_tool_kp = pad(dynamic_tool_kp, max_ntool, dim=1)

            states = np.concatenate([pred_state, dynamic_tool_kp[0:1], static_tool_kp[0:1]], axis=1)
            assert states.shape[1] == max_nobj + max_ntool * 2
            ...
            if vis:
                pred_kp_proj_last, gt_kp_proj_last, gt_lineset, pred_lineset = \
                    ...
                    obj_kp_vis, gt_kp_vis, dynamic_tool_kp, static_tool_kp, Rr, Rs, max_nobj, max_ntool,
...
def rollout(config, epoch):
    ...
    save_dir = f"vis/rollout-{run_name}-model_{epoch}"
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--epoch', type=str, default=500)
