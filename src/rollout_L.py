...
def visualize_graph(data_dir, episode_idx, start, end, vis_t, save_dir,
    ...
    if gt_lineset is None:
        ...
        gt_lineset_new = []
        pred_lineset_new = []
        for li in range(len(gt_lineset)):
            if gt_lineset[li][-1] >= vis_t - t_line:
                gt_lineset_new.append(gt_lineset[li])
                pred_lineset_new.append(pred_lineset[li])
    ...
    for _ in range(1):
        ...
        img_path = os.path.join(data_dir, f'episode_{episode_idx}', f'{start}_color.jpg')
        img_orig = cv2.imread(img_path)
        img = img_orig.copy()
        # intr = intr_list[cam]
        # extr = extr_list[cam]
        save_dir_cam = os.path.join(save_dir)
        os.makedirs(save_dir_cam, exist_ok=True)

        # visualize keypoints
        obj_kp_proj = kp_vis
        obj_kp_proj = np.array(obj_kp_proj)
        ...
        for k in range(len(pred_lineset)):
            ln = pred_lineset[k]
        ...
        gt_kp_proj = np.array(gt_kp_proj)
        ...
        # visualize lineset
        gt_kp_last = gt_kp_proj_last
        if not (gt_kp_last is None):
            gt_kp_last = np.array(gt_kp_last)
            for k in range(gt_kp_proj.shape[0]):
                gt_lineset.append([int(gt_kp_proj[k, 0]), int(gt_kp_proj[k, 1]), int(gt_kp_last[k, 0]), int(gt_kp_last[k, 1]), 
                                int(colormap[k, 2]), int(colormap[k, 1]), int(colormap[k, 0]), vis_t])
    ...
    pred_kp_proj_last = pred_kp_proj_list[0]
    gt_kp_proj_last = gt_kp_proj_list[0]
...
def construct_graph(dataset, n_his, pair, episode_idx, all_particles_pos, all_tool_states):
    ...
    for i in range(len(pair)):
        ...
        obj_kp = all_particles_pos[episode_idx][frame_idx] # (2, num_obj_points, 3)
    ...
    ptcl_cnt = 0
    for j in range(len(fps_idx_list)):
...
def rollout_from_start_graph(graph, model, device, dataset, episode_idx, current_start, current_end, 
    ...
    with torch.no_grad():
        for i in range(1, 1 + rollout_steps):
            ...
            n_frames = len(list(glob.glob(os.path.join(dataset['data_dir'], f"episode_{episode_idx}/*_color.jpg"))))
            ...
            # gt_state = [gt_state]
            gt_state_all = all_particles_pos[episode_idx][current_end] # (2, num_obj_points, 3)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--config', type=str, default='src/config/merging_L.yaml')
