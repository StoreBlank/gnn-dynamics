...
def construct_graph(dataset, n_his, pair, episode_idx, physics_param, material_config, all_particles_pos, all_tool_states):
    ...
    # print(f"obj_y_max: {obj_kp_start[:, 1].max()}; obj_y_min: {obj_kp_start[:, 1].min()}")
...
def rollout_from_start_graph(graph, model, material_config, device, dataset, episode_idx, current_start, current_end, 
    ...
    with torch.no_grad():
        for i in range(1, 1 + rollout_steps):
            ...
            # print(f"pred_state_y_max: {pred_state[:, :, 1].max()}; pred_state_y_min: {pred_state[:, :, 1].min()}")
            ...
            use_chamfer = True
            if use_chamfer:
                error = chamfer(obj_kp, gt_state_all[0])
                error_baseline = chamfer(gt_kp, gt_state_all[0])
